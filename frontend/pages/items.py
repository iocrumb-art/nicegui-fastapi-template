...
def items_page():
    ...
    with dashboard_frame(title="My Crumbs"):
        ...
        with ui.dialog() as dialog, ui.card().classes("min-w-[300px]"): 
            ui.label("Create New Note").classes("text-h6")
            title_input = ui.input("Title").classes("w-full").props('clearable')
            desc_input = ui.textarea("Text").classes("w-full")
            ui.button(
                "Create",
        ...
        ui.button("Add Note", on_click=dialog.open, icon="add").props(
            "color=primary" 
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            with grid:
                ...
                filtered = [item for item in raw_data if item.get('title').startswith('1/')]
                    ...
                    with ui.card().classes("p-0 bg-transparent"):
                            with ui.grid(columns='auto 20px 30px').classes('w-full gap-0'):
                                ...
                                with (
                                    ...
                                    ui.label("Modify Note").classes("text-h6")
				...
				#########################
                                ...
                                ).props("flat dense size=sm color=slate-200")
        else:
...
async def create_item(
    ...
    title_input.value = f'1/{title_input.value}'
    token = state.get_token()
    if not token:
        return
    ...
    try:
        ...
        if response.status_code == 200:
            notifications.show_success("Note created successfully")
...
async def update_item(
    ...
    try:
        ...
        if response.status_code == 200:
            #notifications.show_success("Note updated successfully.")
            dialog.close()
...
async def delete_item(item_id: int, grid: ui.grid):
    ...
    try:
        ...
        if response.status_code == 200:
            notifications.show_success("Note deleted successfully")
