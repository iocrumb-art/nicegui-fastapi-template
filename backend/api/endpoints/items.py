...
@router.delete("/itemsdel/{item_id}", response_model=models.ItemRead)
def del_items(
    *,
    db: Session = Depends(deps.get_db),
    item_id: int,
    ...
    """Retrieves a list of items, either all items for a superuser or
    only the items belonging to the current user."""
    #items = item_repo.get(db=db, id=item_id)
    #if current_user.is_superuser:
    items = item_repo.del_multi_by_owner(db, owner_id=item_id)
    #else:
        #items = item_repo.del_multi_by_owner(db, owner_id=item_id)
    #items = item_repo.remove(db=db, owner_id=item_id)
    return items
